...
> **当前版本：v16**（5个新兵种——刺客/投石手/狂战士/医师/攻城车；完整改动见文末「版本历程」）
...
| 点击己方城堡 + 右键 | 设置单位出生集结点 |
| A + 左键 | 攻击移动 |
| Q / W / E / R / T / Y / U / I / O / P / J / K / L / M | 生产 长矛兵 / 刀盾兵 / 弓箭手 / 骑兵 / 弩手 / 锤子兵 / 骑射手 / 肉盾 / 斥候 / 建筑师 / 刺客 / 投石手 / 狂战士 / 攻城车 |
...
>
> **v9 防御哨塔（v11.1 重装化）**：建筑师（👷）可建造防御哨塔——选中建筑师按 `B` 再左键指定位置，建筑师抵达后施工数秒立起哨塔（消耗 🪵100 🪨100，每阵营上限 8 座）。哨塔是**重装防御塔**（耐久 1500、射程 400、一次齐射两箭），拆塔必须出动攻城单位或集火；请把它放在己方资源点、桥头或基地要道上。**v11.1 基地修复**：基地被打空即「被摧毁」（停火、停出兵），选中建筑师后右键点击被摧毁的基地即可派其修复（🪵300 🪨300）。
>
...
>
> **v15 三层指挥链**：双方 AI 重构为**三层指挥架构**——**主将**（战略意图，~20s）、**军团长**（战术组织，7–10s/军团）、**队长**（确定性执行，持续）。系统根据单位数量自动分配军团长（默认1个，每50人+1个，最多4个），每个军团均衡混编各兵种。主将发布抽象号令（如"长矛手方阵推进中路"），军团长翻译为兵种级战术动作（gather/advance/attack/phalanx/shield_wall/protect_flanks/kite/charge 等），队长最终执行为具体单位命令。军队不再混乱无序——长矛兵结阵推进、骑兵保护侧翼、弓箭手后退射击，有组织有纪律。各角色决策以「【主将】/【军团长1】/【军团长2】」前缀显示在两侧消息条。
>
...
- **单位 skill 化**：每种单位是 `public/js/units/*.js` 下的自包含定义（数值 + 克制 + 标签 + 绘制 + AI 介绍），运行时走 `RTS.Units` 注册表。当前 15 兵种：长矛兵 / 刀盾兵 / 弓箭手 / 弩手 / 骑兵 / 锤子兵 / 骑射手 / 肉盾 / 斥候 / 建筑师 / 刺客 / 投石手 / 狂战士 / 医师 / 攻城车。
- **地图 skill 化**：每张地图是 `public/js/maps/*.js` 下的自包含定义（尺寸 small/medium/large + 基地 + 通道 + 资源 + 地形生成）。当前 3 张：河谷三路（小）、广域河谷（中）、大盆地（大）。
- **大模型完全接管 AI（v15 三层指挥链）**：战略状态（`phase`）100% 来自所选大模型的 `stance`/`attackNow`，连续刷新（~20s）。**主将**定战略意图并给军团长下达抽象指令；**军团长**（自动分配，每50人一个）将指令翻译为兵种级战术动作（phalanx/shield_wall/protect_flanks/kite/charge 等13种动作）；**队长**（确定性逻辑）最终执行为具体单位命令。39 种指挥态势作为低层执行器的「指令集」兜底。支持 DeepSeek、豆包、小米MiMo三家供应商，主菜单为玩家/敌方分别选择；玩家可随时点顶部按钮把自己的部队交给 AI 接管（AI vs AI），也可在主菜单勾选开局即接管。AI 决策以常驻消息条呈现（玩家左蓝 / 敌方右红，最多 5 条，角色前缀区分）。v9 分队指令（`squad` 字段）兼容保留。
...
└── public/
    ...
    └── js/
        ...
        ├── towers.js         # v9 防御哨塔（建筑师建造/自动射箭/销毁；v11.1 重装化）
...
21. **v11 多基地**：每方阵营有 `faction.bases`（数组）与 `faction.base`（=bases[0] 主基地，兼容旧代码）。出兵轮转由 `faction.spawnBaseIdx` 驱动（`Production.decideOrigin` 给订单打 `origin.baseIndex`，`spawnUnit` 按它选出生基地）；胜负判定是「全部基地被摧毁」（`main.js checkEnd`）；每座基地独立角塔防御/集结点/血条（`resources.baseDefenseUpdate` 遍历 `bases`）。改地图时在定义里加 `playerBases`/`enemyBases`（第一项为主基地），单基地地图无需该字段。**AI 侧默认以主基地为中轴**（集结点/方向判断），防守/集火/分路进攻已按多基地适配（`basesOf`/`laneTarget`/`focusFire`）。
...
- **v11**：**多基地 + AI 指挥链调优**——
  - **多基地**：中地图「广域河谷」每方 2 座基地（中路+上路）、大地图「大盆地」每方 3 座基地（上/中/下三路各一座），小地图保持单基地；**出兵按「中基地 → 上基地 → 下基地」轮转**（同时点三次出兵卡片，先从中路基地出、再上路、再下路），每座基地独立集结点、角塔防御与血条，**摧毁敌方全部指挥所才获胜**；中/大地图金矿大幅增加（中图 4→8、大图 8→12，每方 4/6 座安全区金矿），双方都能快速暴兵，避免资源被单方垄断后压着打；AI 分路进攻自动锁定该路对应的敌方基地、围城优先集火血量最低的基地。
  - **主将低频决策**：主将决策频率从 3–6s 降至约 **20s 一次**（副将 4–7s、军需官 5–9s 不变），战略意图稳定、不再朝令夕改，部队不再中途折返。
  - **副将订单协调**：防守副将先于进攻副将落地订单，两份订单统一经协调器落地为逐单位微指令，且按**角色互斥**（各副将的 roster 与选人排除另一副将已占用的单位）——每个单位同一时刻只有一个领导，杜绝「一单位多领导」的拉扯混乱。
  - **斥候行为修正**：斥候占领资源点后需**完全占领（控制值 ±1）+ 无敌人**并驻留 `aiScoutCaptureSettleTime`（4s）才奔赴下一个据点；**斥候不主动交战**（抢点途中不索敌，被攻击后才进入反击窗口），以发挥其全场最快移速的侦查/占点本职；**AI 阵营斥候上限 3 个**（场上+队列），杜绝「斥候人海冲锋」。
- **v11.1**：**基地摧毁/修复机制 + 哨塔重装化**——
  - **基地可被摧毁**：基地血被打空即进入「被摧毁」状态（渲染为废墟、残骸仍占位）：**角塔停火、停止出兵**（出兵轮转自动跳过被摧毁基地），修复前该基地完全瘫痪；**必须派建筑师（👷）修复**——选中建筑师后**右键点击被摧毁的基地**（消耗 🪵300 🪨300，施工 8s），修复完成后耐久恢复到 50% 并恢复出兵与防御；AI 筑垒节奏**优先派建筑师修复被摧毁基地**（高于建兵营/哨塔），无建筑师时自动补产；主将与军需官大模型的请求体新增被摧毁基地数/列表，提示词明确「基地被摧毁→第一时间派建筑师重建」。
  - **防御哨塔重装化**：耐久 420→**1500**、射程 290→**400**、单箭伤害 13→**30**、每轮 **2 箭齐射**、造价 60→**100**——塔成为真正的重装防御建筑（约兵营级耐久、大片防线覆盖），拆塔必须出动攻城单位或集火；AI 建筑师德留数 2→**3**，哨塔选址候选覆盖**每座基地的两侧翼**（base1_l/base1_r…），军需官提示词强调「在地图各处广泛布塔」。`towerBuildCost`/`towerDefense*` 可在 `public/js/config.js` 调参。
- **v11.2**：**并行生产 + 按局势建塔**——
  - **并行生产**：多基地/多兵营下**可同时训练多个部队**——生产队列前 `productionConcurrencyMax`(5) 个订单并行训练（每个订单从自己的出生点出生），第 6 个起排队等待空出的训练槽；再也不用「一个基地一次只训一个兵」地排队等。
  - **按局势建塔**：AI 不再把哨塔修在固定位置——新增 `armyAdvantage`（我方兵力-敌方兵力）局势评估：**兵力领先 ≥ `aiTowerFrontArmyLead`(5) 时，建筑师优先把哨塔修到敌方一侧桥头当「进攻桥头堡」**（压制敌方半场），劣势/均势才在己方一侧桥头、基地两侧防守；**优势时建哨塔优先于建兵营**（劣势/均势先补兵营产能）。军需官选址候选新增 `front_*` 前线点位，提示词明确「优势选 front_* 前压、劣势选 choke_*/base*_* 守家」。
- **v11.3**：**基地强化 + 肉盾重装 + 废墟锚点修复**——
...
- **v13**：**全面性能优化——解决大量单位时 CPU/GPU 过载与内存飙升**——
  - **地形离屏缓存（`render.js`）**：地形（草地/森林/岩石/道路）在开局时一次性渲染到离屏 Canvas，每帧仅一次 `drawImage` 贴图取代逐瓦片循环——**对 128×128 大地图，地形绘制开销降低约 90%**。水面波浪动画仍逐帧更新（仅水面瓦片）。
  - **LOD 简化渲染（`render.js`）**：缩放比低于 `LOD_SIMPLE_ZOOM`(0.65) 时，单位以纯色圆点替代完整矢量绘制，尸体简化为小色点——**远距离鸟瞰时跳过大量 Canvas 路径绘制，帧率显著提升**。
  - **空间分桶复用（`combat.js`）**：`rebuildHash()` 不再每帧 `new Map()`，改为清空并复用已有 Map 和数组——**减少每帧 GC 压力，大量单位时尤为明显**。
  - **投射物对象池（`projectiles.js`）**：已消失的箭矢对象和 trail 数组被回收到对象池，新箭矢优先从池中获取——**消除投射物频繁创建/销毁的 GC 开销**。移除逻辑改用 swap-and-pop 替代 `splice`（从 O(n) 降为 O(1)）。
  - **寻路缓冲区预分配（`pathfinding.js`）**：`Float64Array`/`Int32Array`/`Uint8Array` 在首次寻路时预分配并后续复用（`fill` 重置），MinHeap 实例也复用——**避免大量单位同时寻路时频繁分配大内存块**。
  - **分离力平方距离优化（`combat.js`）**：`applySeparation` 先用平方距离快速剔除远邻，仅对近距离邻居调用 `Math.sqrt`——**减少约 70% 的三角函数调用**。
  - **尸体/飘字数量限制（`combat.js`）**：尸体上限 120、飘字上限 60——**防止长时间对局内存无限堆积**。老化逻辑改用 swap-and-pop 替代 splice。
  - **全局单位数量硬上限（`production.js` + `config.js`）**：新增 `globalUnitCap: 250`（双方合计含队列），超过时停止生产——**从源头控制最大单位数量，防止 CPU 过载**。
  - **配置参数**：`globalUnitCap` 在 `config.js` 中可调。
- **v14**：**兵种特色重构 + AI架构优化 + 地图缩放增强**——
  - **兵种玩法特色**：每个兵种增加独特的玩法优势，从数值白板变为有特色的战术单位——
    - **长矛兵**：新增方阵（3个以上长矛兵聚集时自动结阵：护甲+15%、移速-40%、步调整齐，固定加成不叠加）、反冲锋（对冲锋骑兵造成双倍伤害）、长矛刺击（攻击距离比其他近战远）。
    - **刀盾兵**：新增举盾防御（减少40%远程伤害）、盾击（攻击有25%几率眩晕敌人0.8秒）、格挡（有15%几率完全格挡近战攻击）。
    - **骑兵**：新增冲锋暴击（移动速度越快，首次攻击伤害越高，最高+60）、冲锋击退（冲锋时有40%几率击退敌人）、践踏（对步兵造成额外20%伤害）。
    - **锤子兵**：重新设计，新增破甲（无视60%护甲）、眩晕（攻击有30%几率眩晕敌人1秒）、重击（对建筑造成额外50%伤害，对哨塔额外30%伤害）。
  - **地图缩放增强**：允许缩放到更大范围以俯瞰整个地图（`cameraMinZoom` 0.3，`cameraMaxZoom` 3.0），相机平移范围扩大。
  - **AI基地出兵优化**：修复中地图和大地图多基地出兵bug，AI现在可以根据战术需要选择在哪个基地出兵（近战单位优先前线基地，远程单位优先安全基地）。
  - **AI架构重构**：将四级指挥链改为主将+基地副将架构，去除军需官职能。现在每个基地有独立的副将，负责控制该基地的生产和单位操作。
  - **消息显示优化**：游戏页面的提示现在只显示主将的动态，副将的动态隐藏起来，减少信息干扰。
- **v15**：**三层AI指挥链——主将/军团长/队长**——
  - **三层架构重构**：将v14的主将+基地副将架构重构为三层指挥体系——**主将**（战略意图，~20s）、**军团长**（战术组织，7-10s/军团）、**队长**（确定性执行，持续）。解决了之前AI"全地图如同黑社会斗殴、没有任何组织性"的问题。
  - **军团自动分配**：系统根据单位总数自动分配军团长——默认1个军团长，超过50人自动增加1个，最多4个军团。每个军团均衡混编各兵种，保证战术多样性。
  - **抽象指令体系**：主将发布抽象号令（如"长矛手方阵推进中路"），军团长翻译为13种兵种级战术动作——`gather`（聚集）、`advance`（推进）、`attack`（进攻）、`retreat`（撤退）、`defend`（防守）、`scatter`（分散）、`flank`（侧翼）、`hold`（待命）、`phalanx`（长矛方阵）、`shield_wall`（盾墙）、`protect_flanks`（骑兵护翼）、`kite`（远程风筝）、`charge`（骑兵冲锋）。
  - **队长确定性执行**：队长（确定性逻辑）将抽象动作翻译为具体单位命令——例如"phalanx"会让长矛兵紧密排列向前推进，"protect_flanks"会让骑兵移动到军团两翼，"kite"会让弓箭手边退边射。
  - **有组织的军队**：长矛兵结阵推进、刀盾兵盾墙防御、骑兵保护侧翼、弓箭手后退射击——军队从"黑社会械斗"变成有组织有纪律的军队。
  - **消息显示**：主将和军团长的决策显示在两侧消息条（主将蓝色/军团长绿色前缀）。
  - **命令频率分层**：主将约60秒一条命令（战略稳定）、军团长约30秒一条（战术调整）、队长约10秒执行一次（适应战场变化）——避免命令过于频繁导致部队混乱。
  - **小地图交互**：支持点击小地图快速移动视角、拖拽小地图连续移动——RTS基础功能补全。
  - **配置参数**：`aiCorpsSizeThreshold`（军团人数阈值50）、`aiMaxCorps`（最大军团数4）、`aiCorpsCommanderIntervalMin/Max`（军团长调用间隔25-35s）、`aiSquadLeaderInterval`（队长执行间隔10s）、`aiDecisionIntervalMin/Max`（主将调用间隔55-65s）在 `config.js` 中可调。
- **v16**：**5个新兵种——刺客/投石手/狂战士/医师/攻城车**——
  - **兵种总数扩展至15个**：新增5个特色兵种，每个都有独特的玩法机制，丰富战术选择。
  - **🗡️ 刺客（assassin）**：暗影杀手型近战单位。特色能力：1）隐身——移动时进入隐身状态，不会被敌方自动索敌，但移速降低到70%，靠近敌人120px会被发现；2）伏击暴击——脱离隐身的首次攻击造成2.5倍伤害；3）毒刃——攻击附带毒伤，每秒5点伤害持续3秒。克制弓箭手/弩手/骑射手/医师（×2.0），适合绕后切入秒杀脆皮远程。
  - **🪨 投石手（slinger）**：远程AOE范围伤害单位。特色能力：1）溅射——攻击造成范围伤害，半径60px内敌人受到溅射（中心100%，边缘50%）；2）破阵——溅射命中越多目标伤害越高，每多一个目标+10%，最高+50%；3）攻城——对建筑造成额外30%伤害。对长矛兵×1.3、刀盾兵×1.2，克制密集步兵群。
  - **⚔️ 狂战士（berserker）**：狂暴型近战单位。特色能力：1）狂暴——生命越低攻击力越高，生命低于30%时达到最高+80%攻击力；2）嗜血——击杀敌人恢复40生命，攻速+30%持续5秒；3）战吼——周围有友军时获得攻击力加成，每个友军+5%，最高+25%。适合混战收割，越战越强。
  - **💊 医师（healer）**：战场治疗支援单位。特色能力：1）治疗——每次治疗恢复友军25生命；2）群体治疗——治疗会溅射到目标周围80px内友军，恢复40%治疗量；3）战地急救——对生命低于30%的友军治疗量+50%。不攻击敌人，专注治疗友军，提升部队持续作战能力。
  - **🏗️ 攻城车（siege_ram）**：专门攻击建筑的重型攻城单位。特色能力：1）攻城——对建筑造成巨额伤害（基地150、哨塔200、兵营180）；2）破门——无视80%建筑护甲；3）不能攻击单位，只能攻击建筑。移速极慢、需要肉盾保护才能安全接近建筑。
  - **快捷键分配**：刺客J、投石手K、狂战士L、医师P、攻城车M，生产面板自动显示。
  - **server.js修复**：修复重复函数定义导致的语法错误，AI主将消息通知恢复正常。
